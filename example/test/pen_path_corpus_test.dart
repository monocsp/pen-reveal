...
///   짝짓기(transition system)를 넣은 뒤 **0** 이 됐다 — 교차로에서 가장 덜 꺾는 팔을
///   실제로 그려지는 순서에 적용한 결과다.
///
...
///
///   ⚠️ **map_special_01 의 45 는 성격이 다르다 — 결함이 아니다.** 붙인 교차점에서
///   두 획이 24px 겹치는데, 펜이 그 구간을 **두 번 지난다**(내려오는 획 한 번, 꼬리로
///   나가는 획 한 번). 두 번째로 지날 때는 이미 칠해져 있어 **그림이 변하지 않는다** —
///   실제 손으로 그려도 같다. 그래서 시각 순으로 세면 그만큼 건너뛴 것으로 보인다.
///   붙이기를 끄면 이 값이 17.5 로 내려가지만 **되돌아감이 0 에서 5 로 늘어** 교차로에서
///   반대로 꺾는다. 방향이 맞는 쪽을 택했다.
