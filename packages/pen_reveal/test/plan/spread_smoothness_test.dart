...
//
//   ⚠️⚠️ **이 시험만으로는 부족하다.** `_brushSweep` 이전에 이 시험은 통과하고 있었는데
//   그동안 정본 deep 계열에서는 선단 계단이 지도당 192~477개씩 나 있었다. 합성 획은 자기
//   위를 다시 지나가지 않아서다 — 정본 길은 고리를 그리며 앞서 그은 데를 밟고, 문제는 전부
//   거기서 났다. 그 부류는 `example/test/road_smoothness_corpus_test.dart` 가 잠근다.
//   **여기는 자산 없이 도는 하한선이지 판단 근거가 아니다.**
//
//   상한은 실측값이다(굽기는 결정적이라 값이 안 흔들린다). 좋아지는 것은 통과하고 나빠지는
//   것만 걸린다. 올려야 한다면 연출이 바뀐 것이니 왜 바뀌었는지를 커밋에 적는다.
//
...
void main() {
  ...
  final shapes = <String, (Uint8List, int, int)>{
    ...
    //   붓 자국 합집합이 두 팔의 겹침을 먼저 지나간 시각으로 통일해서 이제 점프가 0 이다
    //   (`_spread` 만 쓰던 때는 81개·최대 38 이었다).
    '머리핀': (
      _stroke(
    ...
    // 뱀꼴 — 굽이마다 소유권 경계가 돈다. (전 97개·최대 37)
    '뱀꼴': (
      _stroke(const [(20, 20), (95, 30), (25, 55), (95, 75), (25, 100)], 7),
      0,
      10,
    ),
    // 굵은 직선 — 이음매가 없다. 보로노이 판이 돌아오면 깨진다.
