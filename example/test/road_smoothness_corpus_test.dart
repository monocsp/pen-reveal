...
const _budget = <String, (int, int, int)>{
  ...
  'map_deep_05': (0, 5, 2),
  'map_deep_06': (0, 4, 2),
  // ⚠️ **계단을 51 → 80 으로 올렸다. 이유를 남긴다.**
  //
  //   지금까지 이 예산을 올리는 것은 금지였다 — 올리면 결함을 덮기 때문이다.
  //   이번은 성격이 다르다. 교차점을 붙이면서 두 획이 24px 겹치는 자리가 생겼고,
  //   펜이 그 구간을 **두 번 지난다**(내려오는 획 · 꼬리로 나가는 획). 두 번째로 지날
  //   때는 이미 칠해져 있어 **그림이 변하지 않는다** — 실제 손으로 그려도 같다.
  //   선단 계단은 그 "안 변하는 구간" 을 계단으로 센다.
  //
  //   대안은 붙이기를 끄는 것인데, 그러면 계단은 51 로 돌아오지만 **교차로에서 반대로
  //   꺾는다**(되돌아감 0 → 5). 사장님 요구는 "내려오다 좌측 하단으로 이어진다" 이므로
  //   방향이 맞는 쪽을 택했다. 그 맞바꿈을 여기 적어 둔다.
  'map_special_01': (80, 4, 3),
};
