...
그 문서는 **스스로를 검증한다.** `corpus_manifest_test.dart` 가 manifest 의 키 목록·굽기 해상도·계보 경로를 실제 코드와 대조하고, 자산이 있는 기계에서는 적어 둔 Flutter 버전이 지금 도는 버전과 같은지까지 본다. 엔진을 올리면 여기가 빨개지는데 그게 의도다 — 리샘플러가 바뀌면 상수를 다시 재야 한다는 신호를 사람의 기억이 아니라 게이트가 준다.
